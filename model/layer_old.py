...
class Gcn(nn.Module):
    ...
    def __init__(self, n_time, n_vertex, n_channel, activation, device):
        super(Gcn, self).__init__()

        if activation == 'glu':
            self.liner = FullyConnection(n_time, n_vertex, n_channel, 2 * n_channel, device)
            self.act = nn.GLU(dim=1)
        elif activation == 'relu':
            self.liner = FullyConnection(n_time, n_vertex, n_channel, n_channel, device)
            self.act = nn.ReLU()
        else:
    ...
    def forward(self, adj, x):
        ...
        :param x: [batch_size, channel, n_time, n_vertex]
        :return: [batch_size, channel, n_time, n_vertex]
        """
        x = torch.einsum('bctv->bcvt', x)
